...
def create_pubsub_mock(mock_pubsub_v1):
    ...
    mock_pubsub_v1.PublisherClient.from_service_account_info.return_value = publisher_mock
...
class TestPubSub(unittest.TestCase):
    ...
    def test_send_message_execute_publish_method_from_pubsub(self, mock_pubsub_v1):
        mock_pubsub_v1 = create_pubsub_mock(mock_pubsub_v1)
        data = {"key": "value"}
        pubsub_instance = PubSub(self.credentials, self.project_name)

        # Assert
        mock_pubsub_v1.PublisherClient.from_service_account_info.assert_called_once_with(
        ...
        mock_pubsub_v1.PublisherClient.from_service_account_info().topic_path.assert_called_once_with(
            self.project_name,
            self.topic_name,
        )

        mock_pubsub_v1.PublisherClient.from_service_account_info().publish.assert_called_once_with(
            expected_topic_path,
