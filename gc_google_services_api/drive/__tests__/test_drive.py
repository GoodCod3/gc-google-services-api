...
class TestDrive(unittest.TestCase):
    ...
    def test_get_permissions_from_file(self):
        ...
        mock_permissions = {
            'permissions': [
                {'id': '1', 'displayName': 'User 1', 'role': 'reader', 'emailAddress': 'user1@example.com'}
            ]
        }
        self.mock_service.permissions().list().execute.return_value = mock_permissions
    ...
    def test_get_shared_drive_by_name(self):
        name = 'Test Drive'
        mock_drives = {
            'drives': [{'id': 'drive_id_1', 'name': 'Test Drive'}]
        }
        self.mock_service.drives().list().execute.return_value = mock_drives
    ...
    def test_set_group_in_shared_drive_permissions(self, mock_sleep):
        ...
        self.mock_service.permissions().create().execute.return_value = mock_permission_result
        success = self.drive.set_group_in_shared_drive_permissions(drive_id, group_email)
    ...
    def test_set_shared_drive_admin_permissions(self, mock_sleep):
        ...
        mock_permission_result = {'id': 'permission_id'}
        self.mock_service.permissions().create().execute.return_value = mock_permission_result
        success = self.drive.set_shared_drive_admin_permissions(drive_id, users)
        self.assertTrue(success)
